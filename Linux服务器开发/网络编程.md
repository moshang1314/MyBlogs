...
> * 当getopt遇到解析问题时，如遇到非法的参数字符或者某个需要参数的命令行项没有参数时，**默认会通过stderr输出错误信息，并将返回值设置为`?`**。若在`optstring`中可能在"+"和"-"之后加入":"，如这样"`-:i:a::bc`"（注意新加入的":"的位置）,则`getopt`不会显式地通过`stderr`输出错误信息，同时在遇到解析错误时，同时在遇到解析错误时，若错误原因为非法参数字符，则 `getopt` 返回 ‘`:`’，而若原因为参数项缺少对应的参数，则 `getopt` 返回 ‘`?`’.
>
> **头文件的相关内容**
>
> ```c
